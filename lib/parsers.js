...
function parseLocation(location, api) {
  ...
  if (api && typeof api !== 'number') {
    result.departuresBoard = { get: function (date) { return api.departuresBoard.get(result.id, date) } };
    result.arrivalsBoard = { get: function (date) { return api.arrivalsBoard.get(result.id, date) } };
  }
  return result;
}
